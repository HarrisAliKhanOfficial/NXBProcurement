...
def all_quotes_verified(id=None):
    ...
    if request_name == 'allCashOrders' :
        orders = cur.execute('SELECT * from orders,user where user.id=orders.staff_id and orders.request_id IS NULL').fetchall()
        orders = items_json(orders)
    elif request_name == 'staffAllCashOrders' :
        orders = cur.execute(f'SELECT * from orders,user where user.id=orders.staff_id and orders.request_id IS NULL and orders.staff_id="{g.user["id"]}"').fetchall()
        orders = items_json(orders)
    elif request_name == 'staffAllOrders' :
        orders = cur.execute(f'SELECT * from orders,user where user.id=orders.staff_id and orders.request_id IS NOT NULL and orders.staff_id="{g.user["id"]}"').fetchall()
        orders = items_json(orders)
    else:
        if g.user['role_id'] == 1:
...
def approve_orderfinance(order_id=None):
    ...
    if request.method == 'POST':
        ...
        if request_name == 'readOrders':
            ...
            orders = cur.execute(f'SELECT * from orders where is_sign=True and is_read={is_read} and orders.request_id IS NULL').fetchall()
            orders = items_json(orders)
            return jsonify(orders)
        else:
            pass
